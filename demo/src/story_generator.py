...
NARRATIVE_STYLES = {
    "personal": {
        "name": "口述回忆录成文风格",
        "description": "第一人称回忆体，像老人面对晚辈口述往事，再整理成正式成文。",
        "prompt_extra": """
叙事风格要求（必须严格遵循）：
- 采用第一人称「我」的视角叙述
- 语气像老人对晚辈娓娓道来，再整理成书面回忆录：朴实、稳当、自然
- 用词以日常中文为主，允许有适度口语感，但不要网络化、不要悬浮抒情
- 要有明确的人生阶段推进：童年家庭、青年经历、转折节点、晚年回望
- 多写具体生活细节、时代背景和个人选择，少写空泛形容
- 情感真挚但克制，不刻意煽情，结尾要有面向后辈的体会或寄语
""",
    },
    "famous_writer": {
        "name": "名家叙事风格（如沈从文等）",
...
class StoryGenerator:
    ...
    def _build_story_prompt(
        ...
        structure_block = """
    成文结构要求（尽量贴近中文人物回忆录样稿）：
    - 开头先从“为什么想把这些经历讲下来”或“看到这张照片想起什么”自然起笔，再交代姓名、年代、籍贯或家庭背景
    - 中间按人生阶段推进，写清楚成长环境、关键选择、吃过的苦、得到的机会、家庭与工作变化
    - 文章主体应形成 4 到 6 个自然段或小节，像完整的回忆录成文，不是短篇随笔
    - 结尾回到今天的生活状态，并总结这一辈子的感受、遗憾、骄傲或留给后辈的话
    - 如材料足够，可以使用“序章 / 第一章 / 第二章 / 第三章 / 第四章 / 第五章”这类小标题；如果材料不够，也要保证段落之间层次清楚
    """
...
3. 输出应是“成文正文”，允许分段，也允许使用简短小标题；但禁止分点、禁止列表、禁止问答体。
4. 文章结构必须体现“照片引出回忆”与“人生阶段展开”两层：不能只停留在这张照片本身，而要写出照片背后的人生历程。
5. 优先保留受访者原话里的生活质感、年代感和身份特征，不要把语言改写得过于像通用散文。
6. 若访谈材料涉及职业、家庭、迁徙、时代变化、退休感受等内容，要把这些节点串起来，写出一个人的完整来路。
7. 字数尽量在 1500-2200 字；如果材料明显不足，也至少写成一篇层次完整、有回望感的回忆录短章。
...
"""
        ...
        style_block = NARRATIVE_STYLES["personal"]["prompt_extra"]
        structure_block = """
多图成文结构要求（必须尽量贴近中文人物回忆录样稿）：
- 把多张照片当作同一个人一生中不同阶段的证据材料来写，不要写成逐张照片说明书
- 开头先交代“为什么想回头讲这一辈子”或“看到这些照片串起了哪些往事”，再点出人物身份、家庭出身或时代背景
- 中间按时间顺序或人生阶段展开，把童年、求学、工作、婚姻、转折、退休等节点串起来
- 各张照片之间要形成前后呼应：上一张照片里的人、地方、工作或情绪，应在下一段继续推进，而不是彼此割裂
- 允许写成 5 到 7 个自然段或带“序章 / 第一章 / 第二章 / 第三章 / 第四章 / 第五章”这类小节的回忆录成文
- 结尾要回到当下生活，用较克制但真挚的语气总结这一生，并给后辈留一句有分量的话
"""

        prompt = f"""请基于以下多张照片的视觉分析和访谈内容，撰写一篇连贯的回忆录成文，串联起跨越时间的人生故事。
...
要求：
1. 成文必须以访谈回答为主，以照片分析为辅。照片只负责引出记忆，真正要写的是人物一生中的经历、选择、关系和感受。
2. 文章要能够自然连接多张照片，展现清晰时间线和人生发展，写出“这张之后为什么会走到下一张”的因果与变化。
3. 每一段都要尽量吸收对应照片下的问答信息，把受访者说过的人物、地点、职业、家庭、时代变化写进去，不能只停留在画面描写。
4. 输出应是“成文正文”，允许分段，也允许使用简短小标题；但禁止分点、禁止列表、禁止问答体。
5. 语言要像老人回望一生时的口述回忆录：朴实、具体、有年代感，有生活细节，但不过度煽情。
6. 要突出照片之间的关联和变化，例如成长、工作变动、家庭责任、社会变迁、晚年回望等。
7. 字数尽量在 1800-2600 字；如果材料不足，也要写成一篇层次完整、具有回望感的回忆录成文。

重要：只输出文章内容，不要输出任何思考过程、解释、元信息或英文内容。
请直接输出文章正文，不要添加标题前缀、说明性开场白或分析文字。"""
